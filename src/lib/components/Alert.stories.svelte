...
<Story
  name="Variants"
  play={async ({ canvas }) => {
    // AC: meaning via icon AND text, never color alone — each banner has a glyph
    // (an svg) alongside its message.
...
<!-- role="alert" for the urgent case — forwarded through ...rest, overriding the
     polite default. -->
<Story
  name="Assertive"
  play={async ({ canvas }) => {
