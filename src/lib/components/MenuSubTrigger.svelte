<script lang="ts">
  ...
  let { children, class: klass, disabled = false }: Props = $props();
</script>

<!-- Opens its `Menu.SubContent`. Same row styling as `Menu.Item`, with a caret affordance. `focus-ring` last so a
