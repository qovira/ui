// `Table` is a compound namespace — `Table.Root`, `Table.Head`, `Table.Body`, `Table.Row`, `Table.HeaderCell`,
// `Table.Cell`, `Table.Caption` — thin styled wrappers over the native table elements, so the semantics (and
// `<th scope>`) come free and consumers compose arbitrary cell content. Mirrors the namespace convention used by
// `Menu`.
