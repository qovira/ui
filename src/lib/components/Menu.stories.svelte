<script module lang="ts">
  ...
  // A modal menu locks `document.body { pointer-events: none }` while open and
  // restores it on close. That body style is shared across stories, so each
  // play must wait for it to settle before ending — otherwise the next story's
  // first click lands on an inert body. Keeps the suite order-independent.
...
<script lang="ts">
  // Per-story open state; each play closes the menu so axe checks the resting
  // (closed) state. Content is portalled into the story host for the scoped
  // `canvas` to see it.
...
<!-- The compound parts render under the `Menu` namespace; Bits supplies the
     menu/menuitem roles, roving focus, type-ahead, portalling, and collision
     handling. The trigger delegates to a Button via `child`. -->
<Story
  name="Dropdown menu"
  play={async ({ canvas }) => {
    onRename.mockClear();
...
<!-- `child` delegation: a menu item rendered as a router-style <a>, keeping the
     menuitem role and the themed classes (passed through Bits' `props`). -->
<Story
  name="Router link"
  play={async ({ canvas }) => {
    ...
    <div id="menu-host-link" class="bg-surface text-fg p-6">
      <Menu.Root bind:open={openLink}>
        ...
        <Menu.Content portalTo="#menu-host-link">
          <Menu.Item>
            {#snippet child({ props })}
              <!-- Demo of `child` delegation to a router-style link; the story
                   asserts the rendered <a>'s attributes and never navigates, so
                   SvelteKit's resolve() isn't in play here. -->
...
<!-- Nested submenu (compound `Menu.Sub`), opened from the keyboard. Grouped
     items carry an accessible group name via `Menu.GroupHeading`. -->
<Story
  name="Submenu"
  play={async ({ canvas }) => {
