...
describe("VARIANTS.destructive", () => {
  ...
  // TDD guard: the old `brightness-95`/`brightness-90` approach dims the entire element
  // including white text, causing contrast to DROP on hover (white-on-#cc4029 ≈ 4.85:1
  // at rest; `brightness-90` would yield ≈ 4.44:1, failing WCAG AA). A CSS `filter`
  // never changes `background-color`, so it is invisible to `getComputedStyle` in
  // browser tests. This test must FAIL on the old code and PASS after the fix.
  ...
  // The fix darkens the background fill via color-mix, leaving text at pure white.
  // This means hover and active each carry a `bg-[color-mix(...)]` arbitrary value
  // while the resting state keeps `bg-error` and `text-white`.
  ...
  // The hover fill must mix in MORE black than the active fill (i.e., active is
  // darker than hover) to produce a visually distinct step.
  it("active mixes in more black than hover (active is darker than hover)", () => {
    // Extract the percentage values from the color-mix expressions. The Tailwind
    // arbitrary-value bracket is `]`; inside the color-mix there are nested parens
    // from `var(--color-error)`, so we use a lazy `.*?` to cross them.
