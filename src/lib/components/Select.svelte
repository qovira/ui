...
<!-- Bits owns listbox behavior: roving focus, type-ahead, ARIA, portalling. The wrapper dresses its parts in
     @qovira/theme utilities and consumes the Field contract on the trigger (the labelable control). `focus-ring` is
     appended last so a consumer class can't strip it. The Root is branched on `type` so the value/onValueChange
     discriminated union narrows to concrete literals — the shared parts live in the `body` snippet to stay DRY. -->
{#snippet body()}
  <Select.Trigger
