...
<!-- Inline status banner: a semantic tint, the matching glyph, and the message. Meaning never rides on color alone —
     the icon (and any title/body text) carries it too. Defaults to a polite role="status"; pass role="alert" via
     ...rest for the rare urgent case (it wins, being spread after). Body copy is text-fg (the brand's body color, never
     text-muted); the icon uses the theme's AA-verified semantic text color on its tint. -->
