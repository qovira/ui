export interface DismissTimer {
  /**
   * Start, or resume after a pause, counting down the time it has left. Idempotent — a call while already running is a
   * no-op.
   */
  resume(): void;
  /**
  ...
  pause(): void;
  /**
   * The fraction of time REMAINING, in [0, 1] (1 = full, 0 = done). While running, this is live — it reflects elapsed
   * wall-clock time so a rAF loop can read it every frame. While paused (or before the first `resume()`), it returns
   * the banked fraction, frozen until the next `resume()`. Returns 0 for a zero-duration timer.
   */
  progress(): number;
}

/**
 * A pause/resume auto-dismiss countdown, decoupled from any framework so the remaining-time arithmetic is unit-testable
 * with fake timers (see `dismiss-timer.test.ts`). `<Toast>` drives it from a single `$effect` keyed on its `paused`
