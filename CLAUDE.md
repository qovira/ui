...
- **Author components in `@qovira/theme` semantic utilities only** (`bg-surface`, `text-fg`, `focus-ring`, `rounded-md`, `text-button`, …).
...
- **`cn()`** (`src/lib/internal/cn.ts`, clsx + tailwind-merge) merges consumer `class` onto the root so consumer classes win conflicts. Extended to treat theme `text-<role>` typography utilities as a `font-size` group so role + color (`text-body text-fg`) don't collapse. Internal — not exported.
...
- **Icons: deep-import `phosphor-svelte/lib/<Icon>`, never the package barrel** — an ESLint rule enforces it (type imports exempt). `icons/index.ts` re-exports the curated set the same way, so consumers still `import { CheckIcon } from "@qovira/ui"`. A barrel import defeats tree-shaking and bloats every consumer's bundle; the why is in `conventions:writing-svelte`.
...
- **`@qovira/theme` peer floor `^2.0.0`.** The components are authored in theme 2.0.0's token vocabulary — foreground `text-fg`/`text-fg-muted` and property-first status utilities `bg-tint-{role}`/`text-fg-{role}` — which the pre-2.0.0 names (`text-text`, `{role}-tint`, `{role}-text`) don't provide; an older theme leaves those classes unstyled. (2.0.0 also carries the 1.0.2 white-on-`error` AA fix — `--color-error` `#cc4029`.) A token-only theme change within the range doesn't force a release here.
